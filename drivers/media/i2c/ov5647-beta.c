...
static bool i2c_neg_disable __initdata = 0;  /* disabled by default */
module_param(i2c_neg_disable, bool, 0);
MODULE_PARM_DESC(i2c_neg_disable, "Disable i2c negotiation");

#define SENSOR_NAME "ov5647-beta"
...
static int ov5647_get_mipi_csi_info(void)
{
	int ret = 0;
	...
	if (0 == ret) {
		...
		mipi_csi2_get_status(mipi_csi2_info);
	}
...
{
	printk(KERN_INFO "%s() in %s: Frame interval is  %d\n",
		       __func__, __FILE__, fie->interval.denominator / fie->interval.numerator);
...
static int ov5647_probe(struct i2c_client *client)
{
	...
	printk(KERN_ERR "%s() in %s: Operation mode: i2c_neg_disable = %d\n",
		    __func__, __FILE__, i2c_neg_disable);

	if (true == i2c_neg_disable)
		goto i2c_neg_completed;
